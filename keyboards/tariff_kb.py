...
async def tariff_price_kb_func():
    ...
    skip_btn = InlineKeyboardButton(text='➡️ Пропустити', callback_data=f'skip:_')
    back_btn = InlineKeyboardButton(text='⬅️ Назад', callback_data=f'back:')
    tariff_price_keyboard.add(tariff_price_1_btn, tariff_price_2_btn, tariff_price_3_btn)
    tariff_price_keyboard.add(skip_btn)
...
async def internet_input_kb_func():
    ...
    internet_input_keyboard.add(skip_btn)
...
async def time_input_kb_func():
    ...
    time_input_keyboard.add(skip_btn)
...
async def connect_type_input_kb_func():
    ...
    connect_type_input_keyboard.add(skip_btn)
...
async def loyalty_input_kb_func():
    ...
    loyalty_input_keyboard.add(skip_btn)
